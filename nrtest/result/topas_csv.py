...
@register('topas binned', 'topas csv', 'topas binary')
class TopasBinnedResult(BaseResult):
    ...
    def __init__(self, *args, **kwargs):
        super(TopasBinnedResult, self).__init__(*args, **kwargs)
    ...
    def _read_header(self, header_str):
        ...
        re_score = '(?P<quant>.+) : (?P<stats>.+)'
        regex_unit = re.compile(re_score_unit)
        regex_no_unit = re.compile(re_score)
        ...
        for line in header_str.splitlines():
            ...
            match = regex_no_unit.search(line)
            if match:
                self.quantity = match.group('quant')
                self.unit = None
                self.statistics = match.group('stats').split()
    ...
    def compatible(self, other):
        ...
        if set(self.statistics) != set(other.statistics):
            return False
        ...
        for stat in self.statistics:
            ...
            with np.errstate(divide='ignore', invalid='ignore'):
                rel_diff = np.where((t == 0) & (r == 0), 0, (t - r) / r)
                rel_diff = np.fabs(rel_diff)

            max_rel_diff = max(max_rel_diff, np.mean(rel_diff))
